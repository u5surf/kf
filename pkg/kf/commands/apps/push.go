  ...
  `,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, app := range appsToDeploy {
				...
				minScale, maxScale, err := calculateScaleBounds(app.Instances, app.MinScale, app.MaxScale)
				if err != nil {
					return err
				}
...
func calculateScaleBounds(instances, minScale, maxScale *int) (*int, *int, error) {
	switch {
	case instances != nil:
		// Exactly
		if minScale != nil || maxScale != nil {
			return nil, nil, errors.New("couldn't set the -i flag and the minScale/maxScale flags in manifest together")
		}
