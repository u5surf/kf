...
func TestDeleteRoute(t *testing.T) {
	...
	for tn, tc := range map[string]struct {
		Namespace string
		Args      []string
		Setup     func(t *testing.T, fakeRoutes *routesfake.FakeClient, fakeApps *appsfake.FakeClient)
		...
		"deleting route fails": {
			...
			Setup: func(t *testing.T, fakeRoutes *routesfake.FakeClient, fakeApps *appsfake.FakeClient) {
				...
				fakeRoutes.EXPECT().Delete(gomock.Any(), gomock.Any()).Return(errors.New("some-error"))
			},
			Assert: func(t *testing.T, buffer *bytes.Buffer, err error) {
		...
		t.Run(tn, func(t *testing.T) {
			...
			fakeRoutes := fakeroute.NewFakeClient(ctrl)
